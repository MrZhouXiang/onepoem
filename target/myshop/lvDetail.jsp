...
<script type="text/javascript">
	...
	function loadPage() {
		...
		userId = Number(getQueryString("userId"));
		madeTypeId = Number(getQueryString("madeTypeId"));
		$.ajax({
					url : "user/getLvDetail",
					data : {
						id : idVar,
						userId : userId,
						madeTypeId : madeTypeId
					},
					dataType : "json",
					type : "post",
					success : function(data) {
						if (data.success) {
							$("#zhizuoLxName").attr("value",
									data.result.zhizuoLxName);
							$("#jingyan").attr("value",
									data.result.jingyan);
							$("#dengji").attr("value",
									data.result.dengji);
	...
	function updateUserLv() {
		
		if(Number($("#jingyan").val()) >= 0){
			$.ajax({
				url : "user/updateLv",
				data : {
					ID : idVar,
					ZhizuoLxName : $("#zhizuoLxName").val(),
					Jingyan : $("#jingyan").val(),
					//Dengji : $("#dengji").val(),
					ZhizuoLxID : madeTypeId,
					YonghuID : userId
				},
				dataType : "json",
				type : "post",
	...
	/* 根据经验获取等级信息 */
	function getLvByExp(){
		$('#jingyan').on("blur",function(){
			var exp = $('#jingyan').val();
			if(Number(exp) < 0 || isNaN(exp)){
				layer.msg("经验要求为大于0的数字, 请重新输入", 2, {
					type : 1,
					shade : false
				});
			...
			$.ajax({
				...
				success : function(data) {
					if (data.success) {
						...
						layer.msg("等级获取失败", 2, {type : 1,shade : false});
					}
				}
			});
		});
...
<body onload="loadPage()">
	...
	<div class="container-fluid">
		<div class="row-fluid">
			...
			<div class="span10">
				<h2>
					等级详情<a href="javascript:history.go(-1);"
						...
						<div class="row-fluid">
							<div class="span12">
								<div class="row-fluid">
									<div class="span4">
										<form id="tab">
											<label>名称</label> <input type="text" value="" id="zhizuoLxName"
												class="input-xlarge" readonly="readonly"> <label>经验</label> <input
												type="text" value="" class="input-xlarge" id="jingyan">
											<label>等级</label> <input type="text" value="" id="dengji"
												class="input-xlarge" readonly="readonly"> 
										</form>
										<div class="btn-toolbar">
											<button class="btn btn-primary" onclick="updateUserLv()">
												 保存
											</button>
										</div>
									</div>
